...
function BranchMaintenance({ isWindowMode = false, onRefresh }) {
  ...
  useEffect(() => {
    ...
    if (!isLoading && !authState.status) {
      ...
      frontendLoggingService.logView("Branch", null, null, "Viewed Branch Maintenance page");
    }
  ...
  // Component content without DashboardWrapper (for window mode)
  const content = isWindowMode ? (
    <div style={{ 
      width: '100%', 
      height: '100%', 
      overflow: 'auto', 
  ...
  // Return content wrapped in DashboardWrapper for regular mode, or unwrapped for window mode
