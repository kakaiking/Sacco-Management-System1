...
// Test endpoint without authentication (for testing purposes only)
router.post("/test", async (req, res) => {
  const transaction = await sequelize.transaction();
  
  try {
    const data = req.body || {};
    const username = "test-user"; // Default test user
...
router.put("/reference/:referenceNumber/approve", validateToken, logUpdateOperation("Transaction"), async (req, res) => {
  ...
  try {
    ...
    await Accounts.update(
      { 
        availableBalance: sequelize.literal(`availableBalance - ${debitEntry.amount}`),
        modifiedOn: new Date(),
        modifiedBy: username
      },
      { 
    ...
    await Accounts.update(
      { 
        availableBalance: sequelize.literal(`availableBalance + ${creditEntry.amount}`),
        modifiedOn: new Date(),
        modifiedBy: username
      },
      { 
